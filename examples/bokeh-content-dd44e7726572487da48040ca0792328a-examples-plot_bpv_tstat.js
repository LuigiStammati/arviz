(function() {
  const fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      const force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      const element = document.getElementById("73343f9a-745b-47fb-bb0b-667a94ecb0d1");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '73343f9a-745b-47fb-bb0b-667a94ecb0d1' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      const js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-mathjax-2.4.1.min.js"];
      const css_urls = [];
      
    
      const inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            const fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  const docs_json = '{"61cc4810-2a4d-467d-829b-6340ab66c4f4":{"defs":[],"roots":{"references":[{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17948","type":"Line"},{"attributes":{"fill_alpha":{"value":0.2},"fill_color":{"value":"#2a2eec"},"hatch_alpha":{"value":0.2},"line_alpha":{"value":0.2},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"17971","type":"Circle"},{"attributes":{"label":{"value":"bpv=0.53"},"renderers":[{"id":"17950"}]},"id":"17967","type":"LegendItem"},{"attributes":{},"id":"17926","type":"UndoTool"},{"attributes":{},"id":"17987","type":"UnionRenderers"},{"attributes":{"fill_color":{"value":"#2a2eec"},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"17969","type":"Circle"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"17961"},"selection_policy":{"id":"17960"}},"id":"17940","type":"ColumnDataSource"},{"attributes":{"coordinates":null,"data_source":{"id":"17940"},"glyph":{"id":"17941"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17943"},"nonselection_glyph":{"id":"17942"},"view":{"id":"17945"}},"id":"17944","type":"GlyphRenderer"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17941","type":"Line"},{"attributes":{"data":{},"selected":{"id":"17963"},"selection_policy":{"id":"17962"}},"id":"17946","type":"ColumnDataSource"},{"attributes":{},"id":"17958","type":"BasicTickFormatter"},{"attributes":{},"id":"17909","type":"LinearScale"},{"attributes":{"coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"17930","type":"PolyAnnotation"},{"attributes":{},"id":"17960","type":"UnionRenderers"},{"attributes":{},"id":"17988","type":"Selection"},{"attributes":{"overlay":{"id":"17930"}},"id":"17925","type":"LassoSelectTool"},{"attributes":{"coordinates":null,"data_source":{"id":"17946"},"glyph":{"id":"17947"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17949"},"nonselection_glyph":{"id":"17948"},"view":{"id":"17951"}},"id":"17950","type":"GlyphRenderer"},{"attributes":{},"id":"17927","type":"SaveTool"},{"attributes":{},"id":"17924","type":"WheelZoomTool"},{"attributes":{},"id":"17918","type":"BasicTicker"},{"attributes":{"coordinates":null,"group":null,"items":[{"id":"17967"}]},"id":"17966","type":"Legend"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#2a2eec"},"hatch_alpha":{"value":0.1},"line_alpha":{"value":0.1},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"17970","type":"Circle"},{"attributes":{"toolbar":{"id":"17993"},"toolbar_location":"above"},"id":"17994","type":"ToolbarBox"},{"attributes":{"line_alpha":0.2,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17943","type":"Line"},{"attributes":{"axis":{"id":"17913"},"coordinates":null,"group":null,"ticker":null},"id":"17916","type":"Grid"},{"attributes":{},"id":"17961","type":"Selection"},{"attributes":{"source":{"id":"17940"}},"id":"17945","type":"CDSView"},{"attributes":{"data":{},"selected":{"id":"17988"},"selection_policy":{"id":"17987"}},"id":"17968","type":"ColumnDataSource"},{"attributes":{"coordinates":null,"formatter":{"id":"17958"},"group":null,"major_label_policy":{"id":"17959"},"ticker":{"id":"17914"}},"id":"17913","type":"LinearAxis"},{"attributes":{},"id":"17905","type":"DataRange1d"},{"attributes":{"source":{"id":"17946"}},"id":"17951","type":"CDSView"},{"attributes":{},"id":"17921","type":"ResetTool"},{"attributes":{},"id":"17959","type":"AllLabels"},{"attributes":{"line_alpha":0,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17947","type":"Line"},{"attributes":{},"id":"17914","type":"BasicTicker"},{"attributes":{},"id":"17956","type":"AllLabels"},{"attributes":{"line_alpha":0.2,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17949","type":"Line"},{"attributes":{"below":[{"id":"17913"}],"center":[{"id":"17916"},{"id":"17920"},{"id":"17966"}],"height":500,"left":[{"id":"17917"}],"output_backend":"webgl","renderers":[{"id":"17944"},{"id":"17950"},{"id":"17972"}],"title":{"id":"17974"},"toolbar":{"id":"17931"},"toolbar_location":null,"width":500,"x_range":{"id":"17905"},"x_scale":{"id":"17909"},"y_range":{"id":"17907"},"y_scale":{"id":"17911"}},"id":"17904","subtype":"Figure","type":"Plot"},{"attributes":{},"id":"17955","type":"BasicTickFormatter"},{"attributes":{"toolbars":[{"id":"17931"}],"tools":[{"id":"17921"},{"id":"17922"},{"id":"17923"},{"id":"17924"},{"id":"17925"},{"id":"17926"},{"id":"17927"},{"id":"17928"}]},"id":"17993","type":"ProxyToolbar"},{"attributes":{"coordinates":null,"formatter":{"id":"17955"},"group":null,"major_label_policy":{"id":"17956"},"ticker":{"id":"17918"}},"id":"17917","type":"LinearAxis"},{"attributes":{},"id":"17911","type":"LinearScale"},{"attributes":{},"id":"17963","type":"Selection"},{"attributes":{},"id":"17962","type":"UnionRenderers"},{"attributes":{"children":[[{"id":"17904"},0,0]]},"id":"17992","type":"GridBox"},{"attributes":{"coordinates":null,"group":null,"text":"y / y","text_font_size":"15pt"},"id":"17974","type":"Title"},{"attributes":{"coordinates":null,"data_source":{"id":"17968"},"glyph":{"id":"17969"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17971"},"nonselection_glyph":{"id":"17970"},"view":{"id":"17973"}},"id":"17972","type":"GlyphRenderer"},{"attributes":{"source":{"id":"17968"}},"id":"17973","type":"CDSView"},{"attributes":{},"id":"17907","type":"DataRange1d"},{"attributes":{"axis":{"id":"17917"},"coordinates":null,"dimension":1,"group":null,"ticker":null},"id":"17920","type":"Grid"},{"attributes":{"overlay":{"id":"17929"}},"id":"17923","type":"BoxZoomTool"},{"attributes":{},"id":"17922","type":"PanTool"},{"attributes":{"tools":[{"id":"17921"},{"id":"17922"},{"id":"17923"},{"id":"17924"},{"id":"17925"},{"id":"17926"},{"id":"17927"},{"id":"17928"}]},"id":"17931","type":"Toolbar"},{"attributes":{"bottom_units":"screen","coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"17929","type":"BoxAnnotation"},{"attributes":{"callback":null},"id":"17928","type":"HoverTool"},{"attributes":{"children":[{"id":"17994"},{"id":"17992"}]},"id":"17995","type":"Column"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17942","type":"Line"}],"root_ids":["17995"]},"title":"Bokeh Application","version":"2.4.1"}}';
                  const render_items = [{"docid":"61cc4810-2a4d-467d-829b-6340ab66c4f4","root_ids":["17995"],"roots":{"17995":"73343f9a-745b-47fb-bb0b-667a94ecb0d1"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    let attempts = 0;
                    const timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (let i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();